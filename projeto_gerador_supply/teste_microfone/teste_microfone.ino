...
const unsigned long tempo_espera = 5000; // Tempo de espera em milissegundos (5 segundos)
...
void loop() {
  ...
  if (!monitorando_barulho && leitura == LOW){
    lcd.clear();
    lcd.setCursor(0,0);
  ...
  if (!gerador_ligado && monitorando_barulho && (tempo_atual - tempo_contagem >= tempo_espera)) {
    gerador_ligado = true;
    Serial.println("Gerador Ligado");
    lcd.clear();
    lcd.setCursor(0, 0);
    lcd.print("Ger.: LIGADO");
  } else if (gerador_ligado && !monitorando_barulho && (tempo_atual - tempo_contagem >= tempo_espera)) {
